...
if (isset($_POST['register'])) {
    try {
        ...
        $stmt = $conn->prepare("SELECT `first_name`, `last_name` FROM `g58_tbl_user` WHERE `first_name` = :first_name AND `last_name` = :last_name");
        $stmt->execute([
        ...
        if (empty($nameExist)) {
            ...
            $insertStmt = $conn->prepare("INSERT INTO `g58_tbl_user` (`tbl_user_id`, `first_name`, `last_name`, `contact_number`, `email`, `username`, `password`, `verification_code`) VALUES (NULL, :first_name, :last_name, :contact_number, :email, :username, md5(:password), :verification_code)");
...
if (isset($_POST['verify'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT `verification_code` FROM `g58_tbl_user` WHERE `tbl_user_id` = :user_verification_id");
        $stmt->execute([
        ...
        if ($codeExist && $codeExist['verification_code'] == $verificationCode) {
            ...
            $conn->prepare("DELETE FROM `g58_tbl_user` WHERE `tbl_user_id` = :user_verification_id")->execute([
                'user_verification_id' => $userVerificationID
            ]);
